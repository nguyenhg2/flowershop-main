      ...
      <div className="container">
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:32,marginBottom:32}}>
          ...
          <div>
            ...
            {CATEGORIES.map(c=><div key={c.id} onClick={()=>navigate('category',{cat:c.id})} style={{cursor:'pointer',padding:'4px 0',fontSize:13,transition:'color .2s'}} onMouseEnter={e=>e.target.style.color='#f7d6df'} onMouseLeave={e=>e.target.style.color=''}>{c.name}</div>)}
          </div>
          <div>
        ...
        <div style={{borderTop:'1px solid rgba(255,255,255,.1)',paddingTop:20,textAlign:'center',fontSize:12,color:'#9a7a68'}}>
          © 2024 Mộng Lan Flower. Tất cả quyền được bảo lưu.
        </div>
      </div>
    </footer>
  );
